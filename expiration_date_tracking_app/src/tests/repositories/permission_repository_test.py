...
class TestPermissionReposiotry(unittest.TestCase):
    def setUp(self):
        ...
        self.employee = user_repository.create(Employee("test_employee", "secret123", emloyer.user_id))



    def test_set_permission_works(self):
        permission_repository.set_permission(self.employee.user_id, self.store.store_id, Permission.VIEW)

        permission = permission_repository.find_permission(self.employee.user_id, self.store.store_id)

        self.assertEqual(permission, "view")

        permission_repository.set_permission(self.employee.user_id, self.store.store_id, Permission.EDIT)

        updated_permission = permission_repository.find_permission(self.employee.user_id, self.store.store_id)
    ...
    def test_find_permission_returns_no_access_if_no_permisson_exists(self):
        permission = permission_repository.find_permission(self.employee.user_id, self.store2.store_id)

        self.assertEqual(permission, "no access")

    def test_find_permission_by_employee_id_works(self):
        permission_repository.set_permission(self.employee.user_id, self.store.store_id, Permission.VIEW)
        permission_repository.set_permission(self.employee.user_id, self.store2.store_id, Permission.MANAGE)

        permissions = permission_repository.find_permissions_by_employee_id(self.employee.user_id)
