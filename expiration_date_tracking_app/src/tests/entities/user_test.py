...
class TestUser(unittest.TestCase):
    def setUp(self):
        self.user = User(username="Merchant",
    ...
    def test_is_employee_works(self):
        user1 = User(username="Merchant", password="Password",
                     role=UserRole.MERCHANT)
        user2 = User(username="Employee", password="Password",
    ...
    def test_hashed_password_is_not_hashed_again(self):
        testuser = User(username=self.user.username,
                        password=self.user.password, role=UserRole.MERCHANT)
        password = self.user.password
        bytes_password = password.encode('utf-8')
        testuser2 = User(username=self.user.username,
                         password=bytes_password, role=UserRole.MERCHANT)
